...
const OrderDetail = () => {
    ...
    if (!order) return <p className="p-20 text-center font-black">FETCHING DETAILS...</p>;

    return (
        <div className="flex bg-slate-50 min-h-screen">
            <Sidebar />
            <main className="flex-1 md:ml-64 p-4 md:p-10">
                <button onClick={() => navigate('/admin-orders')} className="mt-16 md:mt-0 flex items-center gap-2 text-slate-400 font-black uppercase text-[10px] mb-8 tracking-widest">
                ...
                <div className="bg-white rounded-[2.5rem] md:rounded-[4rem] p-6 md:p-12 border shadow-sm max-w-5xl">
                    <div className="flex flex-col md:flex-row justify-between md:items-center border-b pb-8 mb-8 gap-4">
                        <div>
                            <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter uppercase">#{order._id.slice(-6)}</h1>
                            <p className="text-slate-400 font-bold uppercase text-[10px] mt-2">Order Tracking Details</p>
                        </div>
                        <span className="bg-slate-900 text-[#d4af37] px-6 py-2 rounded-full font-black text-xs uppercase tracking-widest w-fit">
                            {order.status}
                        </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16">
                        ...
                        <div className="space-y-8">
                            <section className="bg-slate-50 p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-slate-100">
                                ...
                                <div className="space-y-5">
                                    <p className="flex items-center gap-3 font-black text-xl uppercase italic"><User size={20} className="text-[#d4af37]" /> {order.user.name}</p>
                                    <p className="flex items-center gap-3 font-bold text-slate-500"><Phone size={18} className="text-[#d4af37]" /> {order.user.mobile}</p>
                                    <div className="flex items-start gap-3 font-bold text-slate-500 text-sm leading-relaxed">
                                        <MapPin size={18} className="text-[#d4af37] mt-1 shrink-0" /> <span>{order.address}</span>
                                    </div>
                                </div>
                            </section>
                        ...
                        {/* Order Summary Bag */}
                        <div className="bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-10 text-white shadow-2xl">
                            ...
                            <div className="space-y-6">
                                {order.items.map((item, i) => (
                                    <div key={i} className="flex justify-between items-center border-b border-slate-800 pb-5">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 bg-white rounded-lg p-1 shrink-0"><img src={`http://localhost:5000${item.imageUrl}`} className="w-full h-full object-contain" alt="" /></div>
                                            <div><p className="font-black text-xs md:text-sm uppercase">{item.name}</p><p className="text-[9px] font-black text-[#d4af37] uppercase">{item.selectedSize}</p></div>
                                        </div>
                                ...
                                <div className="pt-6 flex justify-between items-center border-t border-slate-800">
                                    <span className="text-[10px] font-black text-slate-500 uppercase">Total Revenue</span>
                                    <span className="text-3xl md:text-4xl font-black text-[#d4af37] tracking-tighter italic">₹{order.totalAmount}</span>
                                </div>
                            </div>
                        </div>
                    </div>
